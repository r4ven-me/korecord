...
- One `.rec` archive per session -- see [What gets saved, and
  where](#what-gets-saved-and-where) -- with zstd compression and
  password-based encryption both optional and independent of each other.
...
- Indexes every session (start/end time, host, tty, size, exit code,
  compressed/encrypted) in a local SQLite database for fast listing and
  filtering.
...
- Survives crashes: a closed terminal, a crashed window manager, or `korec`
  itself dying mid-recording never leaves an orphaned asciinema process or
  loses more than a fraction of a second of output -- see [Long-running
  sessions and crash safety](#long-running-sessions-and-crash-safety).
- Colored, bordered output ([rich](https://github.com/Textualize/rich))
  wherever it helps interactively (`ls`, `show`, `--help`, ...) -- plain,
  script-friendly text the moment it's piped or redirected.
- Optional password-based encryption at rest (off by default) -- see
  [Optional encryption](#optional-encryption).
- Optional zstd compression (on by default) -- see [Optional
...
the config file, and so nothing external whose loss could ever orphan
already-encrypted data. A side effect of each file being fully
self-contained: different sessions are free to use different passwords --
`korec` just asks again if the one it has on hand doesn't open a
particular one, rather than giving up. There's no password-*change*
support across many sessions at once, though: `encrypt`/`decrypt` always
use whatever password is currently configured/resolvable for that one
session, so there's no bulk migration if you want to move everything to a
new password.
...
finished session. Whether a session is encrypted isn't identifiable from
its filename at all -- an encrypted session's `.rec` looks exactly like a
plain one from a file listing; only the index
...
`make release` runs the test suite, commits anything pending, pushes the
current branch, then tags and pushes `v<version>` (read straight out of
`pyproject.toml`) -- refusing if that tag already exists, so a forgotten
version bump fails loudly instead of silently re-tagging. Pushing the tag
is what actually triggers the workflow below; see the
[`Makefile`](Makefile) for the exact steps, or run them by hand:
...
Developed by Ivan Cherniy -- [r4ven.me](https://r4ven.me).

Source and issues: [github.com/r4ven-me/korecord](https://github.com/r4ven-me/korecord).
