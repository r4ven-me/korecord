...
def test_parse_transcript_line_missing_tab_returns_no_timestamp():
    """A line with no tab at all -- malformed input -- must still be
    greppable/cat-able, just without a timestamp rather than raising."""
    assert db._parse_transcript_line("plain line, no tab") == (None, "plain line, no tab")


def test_parse_transcript_line_unparseable_head_returns_no_timestamp():
...
def test_grep_transcript_line_without_timestamp_prefix_still_matches(tmp_path, capsys):
    """A "txt" member with a line missing the `<seconds>\\t` prefix (e.g.
    corrupted, or hand-edited) must still grep fine, showing '?' instead
    of a time rather than breaking the whole search."""
    path = tmp_path / "malformed.rec"
    archive.create(path, "cast", pack_bytes(b"unused"))
    archive.append(path, "txt", pack_bytes(b"a line with no timestamp prefix\n"))
    sid = db.insert_pending_session(
        start="2026-01-01T10:00:00+00:00", local_host="l", remote_host="r",
        tty="t", path=str(path), pid=os.getpid(),
    )
    db.finalize_session(sid, end="2026-01-01T10:01:00+00:00", duration=60, cast_size=1, exit_code=0)
    assert db.grep_sessions("no timestamp prefix") is True
    out = capsys.readouterr().out
    assert "[?]" in out
...
# --- encryption -----------------------------------------------------------
#
# Each encrypted member carries its own embedded salt (see crypto.py) --
# nothing lives in config.json but the password, so sessions are free to
# use different passwords, and there's no external state whose loss could
