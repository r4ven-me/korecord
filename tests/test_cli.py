...
# --- regression: config.json used to hold one shared salt for every
# encrypted session. Lose that file, re-run `encryption enable` with the
# exact same password, and the salt that comes back is a *new*, different
# one -- silently orphaning every session encrypted under the old one,
# even though the password was correct. Caught for real: a config file got
# deleted, the password was re-entered correctly, and `korec play`/`cat`
# still failed with InvalidTag on every previously-encrypted session. The
# fix is architectural, not a guard: each file now carries its own salt
# embedded directly in it (see crypto.py), so there's no shared state left
# to lose in the first place -- this test proves the actual incident can't
# happen anymore, rather than checking for a warning message. -------------
...
def test_lost_config_file_recovers_with_same_password(tmp_path):
    ...
    # Simulate the actual incident: config.json (and whatever it held) is
    # just gone.
