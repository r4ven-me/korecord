...
lose. An earlier version of this kept one shared salt in the config file
instead, which seemed like a natural place for it -- until that file got
lost separately from the data, and re-entering the exact right password
still couldn't decrypt anything, because a new (different) salt had been
generated in its place. A salt has to match precisely what encrypted the
data; embedding it removes the possibility of it ever going missing on
its own. It doesn't need to be secret -- only the password does -- so
there's no security cost to this, and each file being fully self
-contained also means different sessions are free to use different
passwords, and korec can just ask again if the one it has on hand doesn't
...
-- which does mean every file pays that cost independently now (no shared
key to reuse across a `korec grep` touching many encrypted sessions), a
