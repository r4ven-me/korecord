...
#
# Note there's no salt stored here (an earlier version kept one shared
# salt in the config file, which turned out to be a real footgun -- losing
# that file, or corrupting it, orphaned every encrypted session even with
# the exact right password re-entered, since a salt has to match precisely
# what encrypted the data). Each file now carries its own salt embedded
